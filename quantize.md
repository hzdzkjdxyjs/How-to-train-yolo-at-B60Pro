# 本章节请在阅读deploy后再使用，（仅仅是实验性质的不建议实际使用）

# YOLO 模型（FP32 vs INT8）精度与速度对比表（含差异 ↑↓）

## 📌 说明
- **基线模型：PyTorch FP32（xpu）**
- 所有差异均为：`当前模型数值 - PyTorch FP32`
- INT8（PTQ）来自两种路径：  
  - **ONNX → OpenVINO INT8**  
  - **OpenVINO NNCF INT8**
...
| 指标 | PyTorch FP32（xpu） | OpenVINO FP32（GPU.0） | ONNX → OV INT8（PTQ） | OV NNCF INT8（PTQ） |
|------|----------------------|--------------------------|------------------------------|------------------------------|
| **Precision (P)** | **0.612** | 0.622（↑ +0.010） | **0.00102（↓ -0.611）** | **0.00118（↓ -0.611）** |
| **Recall (R)** | **0.406** | 0.393（↓ -0.013） | **0.019（↓ -0.387）** | **0.0105（↓ -0.395）** |
| **mAP50** | **0.471** | 0.445（↓ -0.026） | **0.000906（↓ -0.470）** | **0.000872（↓ -0.470）** |
| **mAP50-95** | **0.324** | 0.311（↓ -0.013） | **0.000444（↓ -0.323）** | **0.000291（↓ -0.324）** |
| **单张推理（ms）** | **43.3 ms** | **2.2 ms（↑ 19.7×）** | 2.6 ms（↑ 16.7×） | **1.9 ms（↑ 22.8×）** |
| **验证总耗时（s）** | ~6.8 s | **~0.8 s（↑ 8.5×）** | ~0.7 s（↑ 9.7×） | ~0.7 s（↑ 9.7×） |
...
| 模型 | 精度状态 | 说明 |
|------|-----------|---------|
| **OpenVINO FP32** | ✔ 基本保持 | 精度小幅下降但正常 |
| **ONNX → OV INT8** | ❌ 精度彻底崩坏 | mAP 几乎为 0（掉 99.8%） |
| **OV NNCF INT8** | ❌ 精度彻底崩坏 | 同样掉到 0，模型不可用 |
...
- OpenVINO FP32：**加速 20×**
- 两个 INT8：与 FP32 类似速度，但 **精度完全损坏**

---

# 🛠️ YOLO → ONNX → OpenVINO → INT8 量化流程（PTQ）
